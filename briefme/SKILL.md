...
3. **If contrast < 4.5:1, do NOT silently render.** Run the fallback chain *in this exact order*:
   ...
   - **(b) Migrate palette into brand.json.** If `colors.palette` is *missing entirely* (legacy brand.json predating the palette feature), offer to research and populate it once before falling back to derivation. Tell the user: *"Heads up — accent `#X` has ~Y:1 against background, but there's no `colors.palette` in brand.json yet to swap from. Want me to research [client]'s brand palette and add it to brand.json now? One-time migration; future contrast fallbacks will swap to a real brand color automatically."* Offer two choices:
     - **Yes, research and migrate.** Investigate per the "When creating a new brand.json" guidance — look at the client's website, search for brand guidelines, and pull 3–6 brand colors. **Also scan the existing brand.json itself** for non-standard color fields (e.g. `extendedColors`, `secondaryColors`, `palette2`, etc.) — those often contain brand colors that just weren't in the standard `colors` schema. Propose the final palette to the user (one-line list with hex codes), get explicit confirmation, then write the `colors.palette` array into `brand.json`. After the write, retry the palette swap (step a) — it should now succeed.
     - **No, just derive for this run.** Skip the migration and fall through to step (c).
   - **(c) Derived darker variant.** If `palette` is present but no palette member passes 4.5:1, OR the user declined migration in step (b), derive a darker variant of the original accent (HSL lightness -20–30%). Tell the user: *"Heads up — accent `#X` has ~Y:1 against background. No brand-palette color hit 4.5:1, so I derived `#Z` (darker variant of your accent) for this run. Want me to keep `#X` anyway?"*
4. **Run-only swap by default, except for the (b) migration.** The (a) palette swap and (c) derivation are *run-only* — never modify `brand.json` for those. The (b) migration *does* write to `brand.json`, but only after explicit user confirmation of the proposed palette. If the user prefers the original light accent despite the contrast issue, use it and move on — their call.
